...
// *************************************************************************************************
        ...
        for (ofs = beg; (ofs = skipWhitespace(expression, ofs, end)) <= end; ofs++) {
            ...
            for (beg = ofs; ofs <= end; ofs++) {
                ...
                if ( (nxt = getOperator(chr)) != OPERAND) {
                    if (nxt.internal) {
                        nxt = OPERAND;
                    } else { // must kill operator to prevent spurious "Expression ends with a blank sub-expression" at end of function
                        break;
                    }
    ...
    private int opPrecedence (Operator opr, int sid)
    {
        ...
        } else if (opr.unary == NO_SIDE || opr.unary != sid) { // not an operator
            return (sid == LEFT_SIDE ? opr.precedenceL : opr.precedenceR);
        } else { // operator is binary or is unary and bound to the operand on the other side
...
// *************************************************************************************************
    ...
    static class DefaultImpl
        extends Object implements OperatorHandler, FunctionHandler
    {
        private DefaultImpl ()
        {
        }
